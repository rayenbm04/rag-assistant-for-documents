...
┌─────────────────────────────────────────────────────────────────┐
│                        Browser (React)                          │
│   Sessions sidebar │ Chat (SSE stream) │ Documents sidebar      │
└────────────────────────────┬────────────────────────────────────┘
                             │ HTTP / SSE
┌────────────────────────────▼────────────────────────────────────┐
│                     FastAPI Backend                             │
...
┌────────────────────────────▼────────────────────────────────────┐
│                          Ollama                                 │
│   mistral:7b-instruct-q4_K_M  │  nomic-embed-text  │  llava     │
└─────────────────────────────────────────────────────────────────┘
```
...
| Format | Processing |
|--------|-----------|
| PDF | Text extracted with `pdfplumber`; image-only pages (< 50 chars of text) sent to LLaVA for visual analysis |
...
**Multiple sessions**
Unlimited chat sessions, each with its own file list and history. Switching sessions cancels any in-flight request. Sessions persist across page refreshes via `localStorage`.
...
| `GET` | `/documents` | List all uploaded documents with their indexing status. |
| `DELETE` | `/documents/{filename}` | Remove a file and delete its chunks from ChromaDB. |
| `DELETE` | `/documents/all` | Wipe all documents and reset the index. |
...
{ "type": "done",   "sources": ["file.pdf"], "warning": null, "mode": "standard" }
...
├── rag-backend/
│   ├── venv/
│   │   └── main.py              # FastAPI app — all endpoints and RAG pipeline
│   ├── tests/
...
└── rag-frontend/
    ├── src/
    │   ├── App.jsx              # Full React app — sessions, chat, upload, dashboard
    │   └── App.css
    ├── Dockerfile
    ├── nginx.conf
    └── package.json
